...
const FourthScoreCard = ({ batters, bowlers, matchInfoData,liveScore }) => {
  ...
  return (
    <div>
      ...
      <table className='score-table score-table-3'>
        ...
        <tbody>
          {
            batters?.teamD?.batsman?.length > 0 ? batters?.teamD?.batsman.map((bat, i) => (
              <tr key={i}>
                <td style={{ cursor: "pointer" }}>
                <Link to={`/getMatchList/playerInformation/${matchInfoData?.match_id}`}
                className="liveBatterPlayer" state={{
      ...
      <div className="batter-ship flex">
        ...
        <div className='color-gold flex'>
          <p style={{ color: "#D7A700" }}> {batters?.teamD?.extraRun?.extra} </p> <p> (N-{batters?.teamD?.extraRun?.no_ball}, WB-{batters?.teamD?.extraRun?.wide}, LB-{batters?.teamD?.extraRun?.leg_bye})</p>
        </div>
      </div>
      <div className="head-wrapper flex">
      ...
      <table className='score-table score-table-2 score-table-3'>
        ...
        <tbody>
          {
            bowlers?.teamC?.bowler?.length > 0 ? bowlers?.teamC?.bowler?.map((bat, i) => (
              <tr key={i}>
                <td style={{ cursor: "pointer" }} >
      ...
      <div className='partnership-container'>
        ...
        {
          batters?.teamD?.partnerships?.length > 0 ? batters?.teamD?.partnerships?.map((part, index) => (
...
export default FourthScoreCard