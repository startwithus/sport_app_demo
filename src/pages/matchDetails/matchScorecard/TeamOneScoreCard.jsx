...
const TeamOneScoreCard = ({ batters, extraRuns, bowlers, matchInfoData, liveScore }) => {
  ...
  <table className='score-table'>
        ...
        <tbody>
          {
            batters?.teamA?.batsman?.length > 0 ? batters?.teamA?.batsman.map((bat, i) => (
              <tr key={i}>
      ...
      <table className='score-table'>
        ...
        <tbody>
          {
            bowlers?.teamB?.bowler?.length > 0 ? bowlers?.teamB?.bowler?.map((bat, i) => (
              <tr key={i}>
                <td style={{ cursor: "pointer" }} >
                  <Link style={{ color: "white" }} to={`/getMatchList/playerInformation/${matchInfoData?.match_id}`} state={{
                    matchData: matchInfoData,
      ...
      <div className='partnership-container'>
        ...
        {
          bowlers?.teamA?.partnerships?.length > 0 ? bowlers?.teamA?.partnerships?.map((part, index) => (
...
export default TeamOneScoreCard